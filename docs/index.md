...
(2022/4/1追記) アルミ製のプレートに電子パーツが接触して電流が流れ出ないように工夫することで回避は可能です。

対処の例
 -  PCBがたわんで端子がボトムのアルミに接触しないよう黒いレジスト面を内側にして組み立て
 - エンコーダーのボディがプレートに接触しないよう周囲をカプトンテープで保護
 - ネジを伝って流れないよう樹脂製ワッシャをかませる
 - その他 PCBパーツとプレートが接触しそうなところを保護
## 【重要】電流漏洩の対策について
...
## その他の既知の問題点
- 右手側の穴が丸いタイプのカバープレートは、本体側とホールの位置がずれているため使用不能
